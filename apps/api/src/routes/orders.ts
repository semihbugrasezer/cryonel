...
router.post("/", authMiddleware, tradingRateLimit, async (req: Request, res: Response) => {
  try {
    ...
    logger.error("Error creating order", { error: error instanceof Error ? error.message : String(error) });
    res.status(500).json({ error: "Internal server error" });
  }
});

// GET /api/v1/orders - List user's orders
router.get("/", authMiddleware, tradingRateLimit, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
...
router.get("/:id", authMiddleware, tradingRateLimit, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
...
router.put("/:id", authMiddleware, tradingRateLimit, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const orderId = req.params.id;
...
router.delete("/:id", authMiddleware, tradingRateLimit, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const orderId = req.params.id;
